...
- A hook in `.specify/extensions.yml` is explicitly agent-scoped (e.g., Codex-only) — the parity check must respect the scope and not flag the absent skill on the other agent as a failure.
...
- A maintainer manually invokes the underlying PowerShell or Bash script when the agent's slash command is missing — the bridge protocol must record whether this is a sanctioned recovery path or an unsanctioned workaround.
...
- A user explicitly forces a command that the disposition matrix forbids — the override must be logged with the user identity, the policy that was overridden, and the rationale supplied.
...
- **FR-001**: The bridge MUST maintain a machine-readable disposition matrix that lists every Spec Kit command and every Superpowers skill at the verified upstream versions, and assigns each item exactly one disposition: COMBINE, FORBID-UNDER-HANDOFF, SUPERSEDED-BY, or REVIEW-ONLY.
- **FR-002**: Each disposition entry MUST include a human-readable rationale and, where applicable, a pointer to the replacement command or skill.
...
- **FR-004**: The disposition matrix MUST classify `speckit.constitution` and `speckit.checklist` with an explicit decision and rationale.
...
- **FR-007**: For every hook command declared in `.specify/extensions.yml`, the bridge MUST verify that an invocation surface exists for every supported agent OR the hook MUST be explicitly marked as agent-scoped; missing surfaces MUST cause the parity check to fail.
- **FR-008**: Spec Kit commands that have a hook command referencing a non-existent invocation surface for the active agent MUST surface a remediable error to the user, not be silently skipped.
- **FR-009**: Every documented happy-path command MUST be executable from each supported agent via that agent's standard invocation surface (slash commands for Claude Code, `$speckit-*` commands for Codex); falling back to direct script invocation is permitted only as a recorded recovery path with a rationale.
...
- **FR-015**: Overrides — a user explicitly forcing a forbidden action — MUST be logged in `.specify/bridge-events.jsonl` with the overriding identity, the policy that was overridden, and the supplied rationale.

### Key Entities

- **Disposition Entry**: A single record naming a Spec Kit command or Superpowers skill, its disposition (COMBINE, FORBID-UNDER-HANDOFF, SUPERSEDED-BY, REVIEW-ONLY), a rationale, optional replacement pointer, and applicability scope (always / under-handoff / per-agent).
...
- **SC-009**: An override of a FORBID-UNDER-HANDOFF disposition produces a complete log entry (overriding identity, policy reference, rationale) in `.specify/bridge-events.jsonl`.
...
- Hooks currently registered as Codex-only (because Claude Code lacks the corresponding skill) are temporarily acceptable, but each one must be turned into either (a) an agent-scoped hook with an explicit annotation or (b) a hook with a registered Claude Code invocation surface, before the feature is considered complete.
