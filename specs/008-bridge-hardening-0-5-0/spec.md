...
### Session 2026-05-16 (implementation notes — recorded retrospectively per T053)

Decisions that surfaced during Phase A-G implementation, not pre-captured in the Q1-Q6 clarifications above:

- **CI cross-flavor PS detection** (Phase A, post-tag): The first v0.5.0 tag push (workflow run [25958838757](https://github.com/lihan3238/speckit-superpowers-bridge/actions/runs/25958838757)) failed because `tests/test-bridge-state-summary.ps1` hard-coded `Start-Process 'powershell.exe'` which doesn't exist on the Linux CI runner (only `pwsh` is available). Fixed by adding a `Get-PsExecutable` helper that prefers `pwsh` (cross-platform) and falls back to `powershell.exe` then `powershell`. The tag was deleted, the fix committed (`9edaa05`), and the tag re-pushed for run [25958903081](https://github.com/lihan3238/speckit-superpowers-bridge/actions/runs/25958903081) which went GREEN. No spec/plan changes needed — this was a test-portability fix.
- **Sandbox install via unpacked ZIP** (Phase E / T038-T041): The Spec Kit CLI's `specify extension add --from <https-url>` hit a transient `ConnectionResetError` against `github.com` on this dev box (proxy environment issue; not v0.5.0-related). Sandbox was verified by manually downloading the workflow-published ZIP via `gh release download` (SHA256-verified against the workflow Step Summary, `497a3d12…`), unpacking it under `..\test_specify_superpower\extracted\`, and driving the bridge scripts against a synthetic `specs/sandbox-fake-feature/` (3 task-ID lines with 2 outside `## Deferred` and 1 inside). This mirrors the v0.4.2 sandbox precedent recorded in `../003-bridge-cross-platform-scripts/verification.md`. The verification confirmed: (a) installed bridge produces correct `[bridge state]` block; (b) cross-flavor parity holds (PS and bash output byte-identical); (c) FR-019 deliberate-mismatch warning fires on both flavors with exit code 0.
- **Q2=B confirmed in execution** (Phase G / T047-T048): PR `008-bridge-hardening-0-5-0 → main` opened and merged with `gh pr merge --merge --delete-branch` (preserves the 10-commit cycle history on main rather than squashing). The PR carried the entire 003 backlog (7 commits unmerged since v0.4.0) plus the 008 cycle (10 commits) — a single integration moment that closed both the "v0.5.0 release" and the "long-running release-branch consolidation" in one PR. No mid-cycle reshuffling was needed.
- **Working-tree corruption recovery** (Phase G): A misrouted PowerShell `-replace` + WSL invocation chain accidentally deleted most of the source-repo working tree (`.specify/`, `specs/001-007/`, `specs/008-*/{spec,plan,research,data-model,contracts,quickstart}.md`). Recovered cleanly via `git checkout HEAD -- .` (no commits were lost; the damage was working-tree-only). No spec changes triggered. Lesson logged: when scripting bulk file edits on Windows, prefer `sed` (POSIX, reliable on UTF-8) over PowerShell `Set-Content -Encoding UTF8` (which can re-encode em-dashes via Windows console codepage on heredoc-style operations).
- **catalog-update issue rotation completed in-flow**: v0.5.0 issue [#2601](https://github.com/github/spec-kit/issues/2601) filed; v0.4.3 issue [#2600](https://github.com/github/spec-kit/issues/2600) closed with cross-reference comment. Both flows used the issue body in `marketplace/extension-submission-body.md` (filled at file-time with the workflow-emitted SHA256 + workflow URL — file remains committed with `<filled-by-workflow-on-tag>` placeholders for the next cycle).
